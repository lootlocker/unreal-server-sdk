# generate-doxygen-docs.ps1 — LootLocker Unreal Server SDK
# 1. Syncs the mainpage navigation table from groups.dox
# 2. Reads the SDK version from the .uplugin file
# 3. Runs Doxygen
...
# ── 2. Extract version ────────────────────────────────────────────────
$env:LL_SDK_VERSION = (Get-Content "LootLockerServerSDK/LootLockerServerSDK.uplugin" | ConvertFrom-Json).VersionName
